...
---
we need equally spaced datasets for time series analysis. We can't just drop NAs.

Decompose: trend, season
stochastic depends on where you are right now so
hard to detect trends if you have a seasonal 
...
Rainfall_BR_processed <-
  ...
  mutate( Date = my(paste0(Month,"-",Year))) %>% #my is from lubridate- means month year, paste 0 means don't put a space in between 
...
ggplot(BR_complete, aes(x=Date, y= Temperature_C))+geom_line()+geom_smooth()
ggplot(BR_complete, aes(x=Date, y= Rainfall))+geom_line()+geom_smooth()

summary(BR_complete)

# I don't notice any missing data. 
...
# Answer: Temperature is rising while the trend for rainfall is slightly rising. There is seasonality. 
...
f_month<- month(first(BR_complete$Date)) # but you need to make sure that your first observation is the first row. In some cases, the first observation is on the last row!

Temp.ts<- ts(BR_complete$Temperature_C, start=c(1901,1), frequency= 12) #12 because we have monthly data 
...
ggplot(Temp.Components, aes(x=Date)) +
  geom_line(aes(y = Observed),  size = 0.25) +
  geom_line(aes(y = trend), color = "#c13d75ff") +
  geom_hline(yintercept = 0, lty = 2) +
  ylab(expression("Temperature (C)"))

ggplot(Rainfall.Components, aes(x=Date)) +
  geom_line(aes(y = Observed),  size = 0.25) +
  geom_line(aes(y = trend), color = "#c13d75ff") +
  geom_hline(yintercept = 0, lty = 2) +
...
# Run SMK test (way to analyze trends when there's seasonality, non-parametric)
