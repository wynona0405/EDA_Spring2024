...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    ...
    ) #expression is used here because we're using Greek symbols like mu
...
PeterPaul.chem.nutrients$month_f <-factor(x=PeterPaul.chem.nutrients$month,
                                          levels=1:12, labels=month.abb)

#Try again: Plot how many Lake observations occur each month
ggplot(PeterPaul.chem.nutrients, aes(x=month_f))+geom_bar() + scale_x_discrete(drop=FALSE) #this takes data and put it into plot
...
PeterPaul.chem.nutrients%>%
  filter(depth==0)%>%
  ggplot(aes(x=daynum, y=temperature_C, color=year4))+geom_point()+labs(x="Day of the year" ,y="Temperature (C)")%>%
  facet_wrap(PeterPaul.chem.nutrients$lakename) + labs(color="year")
...
PeterPaul.chem.nutrients%>%
  ...
  ggplot(PeterPaul.chem.nutrients, 
         aes(x=sampledate, y=temperature_C, color=depth))+geom_point(size=0.5)
