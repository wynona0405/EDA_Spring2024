...
>Answer: The estimated value of β0 is 0.3888. This corresponds to the log odds of finding disease in a sampled forests.

b. Given the estimate of the intercept, what is the predicted probability (i.e. ˆp) of finding the disease in a forest?
>Answer: The probability of finding the disease in a forest in 0.5959938. 
...
question 1?
>Answer: The estimate of β0 is 0.7053. It represents the log odds of finding the disease in new growth forests. It is almost twice as large as the estimate in question 1.

b. Based on your estimates from the model, what is the predicted probability of finding the disease in an
old-growth forest?
>Answer: The probability of finding the disease in old-growth forests is 0.523807. The log odds of finding the disease in old-growth forests is 0.0953.
...
new-growth forest?
>Answer: 0.6693618 is the probability of finding the disease in a new-growth forest.
...
>Answer: The p-value is 0.01892. Since it is less than 0.05, we reject the null hypothesis of no difference in disease status between new-growth and old-growth forests.

```{r}
t.test(disease~old, data=tree_disease_df)
```
...
>Answer: The odds ratio is 1 which suggests that the odds of finding a disease versus not finding a disease in different basal areas are the same.  
...
>Answer: The p-value is 0.997610, which is higher than 0.05, so the null hypothesis stands.  

```{r}
anova(fit_3)
```

#Question for TA: Can we just look at the p-value in the summary table or do we need to run an additional test?
...
>Answer: The odds ratio for the effect of temperature anomalies on disease status is 1.618361 which suggests that the odds of finding a disease in trees is higher for those with temperature anomalies than without. 
...
The factors associated with increased odds of disease is if the tree is from new-growth forest and if there are temperature anomalies. This suggests that disease thrives in slightly higher temperatures and that old-growth forests are more resilient to disease. I identified these two variables because the log-odds for these variables are not 1 and that the p-values for these variables are larger than 0.05 (unlike basal area). 
