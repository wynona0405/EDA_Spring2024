...
> Answer: It shows that the packages are loaded in blue text. 
...
> ANSWER: We can find this file on a relative file path associatiated with the USGS file site. The best practice is to use a relative file path that can be accessible through any computer and that raw data should not be changed. 
...
> Answer: We can remove NAs when we don't need discharge and gauge height observations to answer the research question. **CHECK! Is this right? 

```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data)
dim(USGS.flow.data) #dim() sets or returns the dimension of the matrix, array, or data frame
